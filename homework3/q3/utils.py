...
def pad_sents(sents, pad_token):
    ...
    ## Identify the maximum length of the sentences in the batch.
    max_len = max(len(sent) for sent in sents)
    
    ## Pad each sentence to that length by appending the pad token.
    for sent in sents:
        padded_sent = sent + [pad_token for _ in range(max_len - len(sent))]
