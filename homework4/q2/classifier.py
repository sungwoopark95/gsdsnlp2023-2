...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE)):
            ...
            if step % 50 == 0:
                print(f"step loss : {(train_loss / num_batches):.5f}")
        ...
        print(
            f"Epoch {epoch+1}: train loss :: {train_loss :.3f}, train acc :: {train_acc :.3f}, dev acc :: {dev_acc :.3f}")
...
def get_args():
    ...
    parser.add_argument("--dev_out", type=str, default="dev-output.txt")
    parser.add_argument("--test_out", type=str, default="test-output.txt")
