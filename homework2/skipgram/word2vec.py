...
def naiveSoftmaxLossAndGradient(
    ...
    # 1. Calculate the softmax loss
    # 2. Calculate the gradient with respect to the center word vector
    # 3. Calculate the gradient with respect to all the outside word vectors

    # 1. Calculate the softmax loss
    # 1.1 Calculate the scores
    scores = np.dot(outsideVectors, centerWordVec)
    # 1.2 Calculate the probabilities
    probs = softmax(scores)
    # 1.3 Calculate the loss
    loss = -np.log(probs[outsideWordIdx])

    # 2. Calculate the gradient with respect to the center word vector
    # 2.1 Calculate the gradient of the loss with respect to the scores
    dscores = probs
    dscores[outsideWordIdx] -= 1
    # 2.2 Calculate the gradient of the scores with respect to the center word vector
    gradCenterVec = np.dot(outsideVectors.T, dscores)

    # 3. Calculate the gradient with respect to all the outside word vectors
    # 3.1 Calculate the gradient of the loss with respect to the outside word vectors
...
def negSamplingLossAndGradient(
    ...
    # Loss and gradient for the true outside word
    z_positive = np.dot(outsideVectors[outsideWordIdx], centerWordVec)
    loss -= np.log(sigmoid_function(z_positive))
    gradCenterVec += (sigmoid_function(z_positive) - 1) * outsideVectors[outsideWordIdx]
    gradOutsideVecs[outsideWordIdx] += (sigmoid_function(z_positive) - 1) * centerWordVec

    # Loss and gradients for negative sampled words
    for k in negSampleWordIndices:
        z_negative = -np.dot(outsideVectors[k], centerWordVec)
        loss -= np.log(sigmoid_function(z_negative))
        gradCenterVec += (1 - sigmoid_function(z_negative)) * outsideVectors[k]
        gradOutsideVecs[k] += (1 - sigmoid_function(z_negative)) * centerWordVec
    ...
    # # 1. Calculate the negative sampling loss
    # # 2. Calculate the gradient with respect to the center word vector
    # # 3. Calculate the gradient with respect to all the outside word vectors
    ...
    # # 1.3 Calculate the loss
    # loss = -np.log(probs[outsideWordIdx]) - np.sum(np.log(1 - probs[negSampleWordIndices]))

    # # 2. Calculate the gradient with respect to the center word vector
    # # 2.1 Calculate the gradient of the loss with respect to the scores
    # dscores = probs
    # dscores_neg = sigmoid(-scores)
    # gradCenterVec = - (1 - dscores[outsideWordIdx]) * outsideVectors[outsideWordIdx]
    # for k in range(K):
    #     gradCenterVec += (1 - dscores_neg[negSampleWordIndices[k]]) * outsideVectors[negSampleWordIndices[k]]

    # # 3. Calculate the gradient with respect to all the outside word vectors
    # # 3.1 Calculate the gradient of the loss with respect to the outside word vectors
    # gradOutsideVecs = np.zeros(outsideVectors.shape)
    # gradOutsideVecs[outsideWordIdx] = - (1 - dscores[outsideWordIdx]) * centerWordVec
    # for k in range(K):
    #     gradOutsideVecs[negSampleWordIndices[k]] += (1 - dscores_neg[negSampleWordIndices[k]]) * centerWordVec
